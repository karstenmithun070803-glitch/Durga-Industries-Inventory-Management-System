  ...
  return (
    <>
            ...
            <div className="overflow-auto flex-1">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {filtered.length === 0 && (
                    <tr>
                      <td colSpan={4} className="px-4 py-8 text-center text-slate-400 text-sm">No units found</td>
                    </tr>
                  )}
                  {filtered.map((unit, i) => (
                    <tr key={unit.id} className="border-t border-slate-100 hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-4 py-2.5 text-right">
                        <div className="flex justify-end gap-1">
                          <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => startEdit(unit)}>
                            <Pencil className="w-3.5 h-3.5" />
                          </Button>
                          <Button variant="ghost" size="icon" className="h-7 w-7 text-red-500 hover:text-red-600 hover:bg-red-50"
                            onClick={() => setDeletingId(unit.id)} disabled={isPending}>
      ...
      <ConfirmDialog
        ...
        title="Delete unit?"
        description="This will soft-delete the unit. Materials using this unit will retain their existing assignments."
        onConfirm={() => {
